...
# remove the rows which the Active Area (cm2) is bigger than 500

# Prepare the data for visualization
df['Active Area (cm2)'] = pd.to_numeric(df['Active Area (cm2)'], errors='coerce')
df = df[df['Active Area (cm2)'] < 500]
df = df[df['Flow Rate (Ml/min)'] < 15]
...
#
# Save the plot
plt.savefig('operation_condition.png', dpi=300)